...
所有的 class 和 interface 类型。比如 String 类就是常见的引用类型。
...
字节流的操作不会经过缓冲区（内存）而是直接操作文件本身的，而字符流的操作会先经过缓冲区（内存）然后通过缓冲区再操作文件。  
...
- 对于操作需要通过 IO 在内存中频繁处理字符串的情况则使用字符流会好些，因为字符流具备缓冲区，提高了性能。
